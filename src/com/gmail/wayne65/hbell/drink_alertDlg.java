 ...
 * @version 1.0.0
 */
public class drink_alertDlg extends Activity {
...
//		mLogTag = alert_dlg.class.getSimpleName();
		...
		new AlertDialog.Builder(arg0).setTitle(strTitle).setMessage(strMessage)
				.setPositiveButton(strPosBtn, new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialoginterface, int i) {
						if (pref.LoadDrinkBellCheckedValue(drink_alertDlg.this) == true){
							publicFun.ringBell(drink_alertDlg.this, false); 	// 停止音樂
						}
						if (mVibrator != null) mVibrator.cancel();			// 停止振動
						
						drink_alertDlg.this.finish();
					}
	...
	private void _alertProcess(){
		if (pref.LoadDrinkBellCheckedValue(drink_alertDlg.this) == true){
			publicFun.ringBell(drink_alertDlg.this, true); // 撥放音樂
		}
		if (pref.LoadDrinkVibrationCheckedValue(drink_alertDlg.this) == true){
			_vibration();
		}
		_showAlertDialog(drink_alertDlg.this, "喝水提醒", "喝水了！", "確認", "略過");
	}
	
	private void _vibration(){
		mVibrator = (Vibrator)getApplication().getSystemService(Service.VIBRATOR_SERVICE);
		mVibrator.vibrate(new long[]{100, 10, 100, 1000}, -1);
		publicFun.showToast(drink_alertDlg.this, "振動中");
	}
	// Inner classes
