 ...
 * @version 1.0.0
 */
public class meal_alertDlg_dinner extends Activity {
...
//		mLogTag = alert_dlg.class.getSimpleName();
		...
		new AlertDialog.Builder(arg0).setTitle(strTitle).setMessage(strMessage)
				.setPositiveButton(strPosBtn, new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialoginterface, int i) {
						...
						if (pref.LoadMealBellCheckedValue(meal_alertDlg_dinner.this) == true){
							publicFun.ringBell(meal_alertDlg_dinner.this, false); 	// 停止音樂
						}
						if (mVibrator != null) mVibrator.cancel();			// 停止振動
						
						meal_alertDlg_dinner.this.finish();
					}
	...
	private void _alertProcess(){
		if (pref.LoadMealBellCheckedValue(meal_alertDlg_dinner.this) == true){
			publicFun.ringBell(meal_alertDlg_dinner.this, true); // 撥放音樂
		}
		if (pref.LoadMealVibrationCheckedValue(meal_alertDlg_dinner.this) == true){
			_vibration();
		}
		_showAlertDialog(meal_alertDlg_dinner.this, "晚餐提醒", "吃晚餐了！", "確認", "略過");
	}
	
	private void _vibration(){
		mVibrator = (Vibrator)getApplication().getSystemService(Service.VIBRATOR_SERVICE);
		mVibrator.vibrate(new long[]{100, 10, 100, 1000}, -1);
		publicFun.showToast(meal_alertDlg_dinner.this, "振動中");
	}
	// Inner classes
